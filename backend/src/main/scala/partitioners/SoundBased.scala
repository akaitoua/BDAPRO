...
import org.apache.commons.text.similarity.{EditDistance, JaccardDistance, JaroWinklerDistance, SimilarityScore}
...
class SoundBased extends Serializable {
  ...
  def produceSimilarity(row1:Array[String],row2:Array[String],compAlg:String,threshold:Double):EntityMatch={
    ...
    for (i <- 1 to dataColsLen ) {
      sim +=  (if(row1(i)!="null" && row2(i) != "null")1 - dm.apply(row1(i), row2(i)) else { decSim();0; })
    }
  ...
  def matchEntities(input1:String,input2:String,output:String,idcols:Array[String],compAlg:String,threshold:Double): Unit ={
